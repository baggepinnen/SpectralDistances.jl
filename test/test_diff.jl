...
using SpectralDistances: ngradient, nhessian, njacobian, polyconv, eigsort, hproots, rev, companion
...
# A = randn(10000,10); y = randn(10000)
#
# ls1(A,y) = A\y
# ls2(A,y) = pinv(A)*y
# ls3(A,y) = (A'A)\A'y
#
# @time Zygote.gradient(A->sum(ls1(A,y)), A)
# @time Zygote.gradient(A->sum(ls2(A,y)), A)
# @time Zygote.gradient(A->sum(ls3(A,y)), A)
